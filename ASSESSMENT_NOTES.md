...
3. Refactor repeated project-access checks using a proposed `@ProjectPermission(VIEW | MANAGE)` decorator, a `ProjectAccessGuard`, and a `@CurrentProjectAccess()` parameter decorator. Keep `ProjectAccessService` as the shared policy and database lookup layer; the guard would run after JWT authentication, resolve the project from either `projectId` or the task identified by `taskId`, and attach the verified project, organization role, and project role to the current request. Pass that context into the operation to avoid repeating the same lookups within the request. Keep operation-specific rules, such as member self-assignment and assignee eligibility, in domain services, and preserve authorization for non-HTTP callers before removing existing checks. Verify equivalent allow/deny behavior for both route shapes and internal entry points, plus lookup reuse. This is a future maintainability improvement, not an implemented feature; project permissions would still come from current database memberships rather than role claims in JWTs.
4. Introduce URI-based API versioning with public routes such as `/api/v1/tasks/:taskId`, allowing future breaking request/response changes to use `/api/v2` while supported clients continue using v1. Define compatibility and deprecation rules; do not increment the API version for every internal refactor or backward-compatible addition. Coordinate NestJS routing, the frontend API base URL, and the single-app Heroku proxy: the current Next.js rewrite strips `/api`, so explicitly align the public and internal paths to avoid missing or duplicated prefixes. Update documentation, API/browser tests, and deployment smoke checks, with a deliberate migration path for existing unversioned clients. This is planned work, not currently implemented versioning.
5. Add operational metrics for assignment failures, transaction retries, slow activity queries, and migration checks. Measure before introducing caching or queues.
6. Improve long-history UX with cursor pagination and refresh indicators, and design membership-removal/retention rules before adding those capabilities.
